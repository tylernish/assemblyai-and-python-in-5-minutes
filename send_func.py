...
def addNewEpisodes(wrks_name, df_to_add):
    print("**addNewEpisodes being run")
    if df_to_add.empty:
...
def addSummaries(wrks_name, episodeName):
    ...
    print("\tEpisode Name is "+str(episodeName)+", Episode Name's Index is: " + str(episodeIndex))
...
#Same thing but just based on the url instead of the episodename
def addSummariesFromURL(wrks_name, episodeURL):
    #Now add in the summary of the transcript
    #location = [10, read.]
    print("**addSummaries being run")
    episodeURLIndex = read.getIndex(str(episodeURL), wrks_name)#need first element (row) out of that 2 length list for the index: [row,col]
    print("\tEpisode Name is "+str(episodeURL)+", Episode Name's Index is: " + str(episodeURLIndex))
    if episodeURL is None or pd.isna(episodeURL):
        print("\tThat episode either does not exist, or there was an issue with saving the episode data to the excel file")
        
    else:
        #print("input for read.getValue("+ 'H' + str(episodeIndex[0])+", " + wrks_name+')')
        transcript = read.getValue('H' + str(episodeURLIndex[0]), wrks_name)
        ##print(transcript)
        summaryIndex = [episodeURLIndex[0], episodeURLIndex[1]-1]
        #send the summaries to the google sheet
        print('\tAdding in the missing summary for the episode')
        updateCell(wrks_name, summaryIndex, summarize_func.summarizeTranscript(transcript))
        print('\tCell updated with summary')
...
def sendMissingRowData(wrks_name, df, row):
    print('**sendMissingRowData being run! On DF row ' + str(row))
    row = row+2
    rowString = str(row)
    ...
    if (pd.notna(episodeNameCell)):
        ...
        print("\tEpisodeNameCell doesn't exist but there is a URL that was submitted from Webflow")
        # if there is no transcript, fill in all other missing data
        transcript = read.getDfValue(row, 'Transcript', df)
        if (pd.isna(transcript) or (transcript is None)):## and read.getValue('B'+rowString)==None):
...
def sendMissingDataFromURL(wrks_name, episodeGoogleURLCell, df, row, rowString):
    print('**sendMissingDataFromURL being run. No transcript but there is a URL. Scraping the rest of the missing data and inserting it')
    ...
    if updatedRow.empty:
        ...
        sectionToUpdate = 'A'+rowString+':I'+rowString
        updateSection(wrks_name, sectionToUpdate, updatedRow)
        # print('''\tHere is all the info going into updateSection()** \n\t\t
        #       updatedRow:'''+str(updatedRow) + '\n\t\tsectionToUpdate:'+str(sectionToUpdate)+'\n\t\tand df:'+str(df))
        # print("\tGoing to wait 20 seconds")
        # time.sleep(60) #Trying to wait a bit here because I think that the episode name is being searched for too soon, might need to move up
        # print("\tDone waiting 20 seconds")
        # ##Try solving the issue by taking the EpisodeName from the UpdatedRow instead of the df
        # episodeNameCell = read.getDfValue(row, 'Episode_Name', df)
        # print("\tEpisode name cell content is: "+str(episodeNameCell) +', for row: '+str(row))
        # addSummaries(wrks_name, episodeNameCell)
        addSummariesFromURL(wrks_name, episodeGoogleURLCell)
...
def sendMissingDataFromEpisodeName(wrks_name, episodeNameCell, row, seriesName, rowString):
    ...
    print('**sendMissingDataFromEpisodeName being run. No transcript but there is a episodeName. Scraping the rest of the missing data and inserting it')
    url = search_func.get_google_podcasts_link(seriesName)
    # $$ this below is returning an empty list or df
...
def updateSheetData(wrks_name):
    print("**updateSheetData being run")
